...
let form = new Vue({
    ...
    methods: {
        addPoint: function () {
            if (this.y !== "") {
                for (const currentX of this.x) {
                    for (const currentR of this.r) {
                        let point = {
                            x: currentX,
                            y: this.y,
                            r: currentR,
            ...
            if (this.r.length === 0) {
                textWindow.message = 'Выберете хотя бы один R!';
            }
            if (this.x.length === 0) {
                textWindow.message = 'Выберете хотя бы один X!';
            }
        ...
        reset: function () {
            pointsApi.remove({}).then(result => result.text().then(data => {
                ...
                error => {
                    textWindow.message = "При удалении результатов возникла ошибка!" + error.body.error;
                    textWindow.response = false;
                    textWindow.lastPoint = [];
                })
        },
        ...
        isValid: function () {
            ...
            if (/^[-]?[0-9]?[.]?[0-9]{0,5}$/.test(this.y)) {
                if (!/[-][.]/.test(this.y)) {
                    msg = 'Y: "' + this.y + '" не принадлежит промежутку [-5..5]!'
